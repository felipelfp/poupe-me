...
const CaminhoGame = {
    ...
    showFinalMessage() {
        ...
        const padding = isMobile ? '14px 16px' : '30px 40px';
        const fontSize = isMobile ? '14px' : '16px';
        notification.style.cssText = `position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: linear-gradient(135deg, var(--gold), #ffeb99); color: #000; padding: ${padding}; border-radius: 14px; font-weight: 900; z-index: 9999; box-shadow: 0 15px 40px rgba(0,0,0,0.6); font-size: ${fontSize}; text-align: center; width: min(${maxWidth}, 460px); pointer-events: all;`;
        notification.innerHTML = `<p style="margin: 0; font-size: ${isMobile ? '15px' : '18px'}; line-height: 1.35; font-weight: bold;">Agora você tem tempo para fazer o que realmente importa e é estratégico.<br><br>O restante deixa com o Poupe-me!</p>`;
        
        // Fundo semi-transparente
        const overlay = document.createElement('div');
        overlay.setAttribute('data-final-overlay', 'true');
        overlay.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 9998; cursor: pointer; pointer-events: auto;';
        ...
        // Permitir fechar clicando no overlay ou notificação
    ...
    showCurrentQuestion() {
        ...
        this.dialogBox.innerHTML = `
            ...
            ${d.allowMultiple ? '<p style="font-size: 11px; color: #ffd700; margin: 4px 0 8px;">Pode escolher até duas opções.</p>' : ''}
            ${optionsHtml}
        `;
        if (d.allowMultiple) {
    ...
    gameLoop() {
        ...
        this.obstacles.forEach((obs, index) => {
            ...
            const isProtectedPhase = this.score < 50; // 5 primeiros obstáculos (5 x 10 pts)
            
            if (!isProtectedPhase && wizardRect.right > obsRect.left + 15 && wizardRect.left < obsRect.right - 15 && wizardRect.bottom > obsRect.top + 15) {
                this.gameOver();
            }
    ...
    showGameOverWithConsultationMessage() {
        ...
        const isMobile = window.innerWidth < 768;
        notification.style.cssText = `position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: linear-gradient(135deg, var(--gold), #ffeb99); color: #000; padding: ${isMobile ? '14px 16px' : '30px 40px'}; border-radius: 14px; font-weight: 900; z-index: 5000; box-shadow: 0 15px 40px rgba(0,0,0,0.6); font-size: ${isMobile ? '14px' : '15px'}; text-align: center; width: min(${isMobile ? '92vw' : '85%'}, 460px); line-height: 1.35;`;
        notification.innerHTML = '<p style="margin: 10px 0; font-size: 18px;">🎉 Parabéns!</p><p style="margin: 10px 0;">Você ganhou uma <strong>CONSULTORIA GRATUITA</strong>!</p><p style="margin: 15px 0; font-size: 14px;">Logo nossos consultores irão entrar em contato com você ou nos chame no WhatsApp.</p><p style="margin: 15px 0; font-size: 14px; font-style: italic;">Agora o comprador é liberado pra você se divertir e bater seu record! 🚀</p><button onclick="this.parentElement.remove(); restartCaminhoRun();" style="margin-top: 20px; padding: 10px 20px; background: #d4af37; color: #000; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; font-size: 14px;">JOGAR DE NOVO</button>';
        document.body.appendChild(notification);
    },
